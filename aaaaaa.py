...
async def material(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.video or update.message.photo or update.message.document:
        ...
        await update.message.reply_text('Ma\'lumotlaringiz to\'g\'rimi? Iltimos, tasdiqlang.')
        return CONFIRM
    else:
...
async def confirm(update: Update, context: CallbackContext) -> int:
    user_id = await update.message.chat_id
    if update.message.text.lower() in ['ha', 'yes']:
        ...
        await update.message.reply_text('Ma\'lumotlar rad etildi. Iltimos, qayta yuboring.')
        return MATERIAL
    return ConversationHandler.END
